# Shared Alerts — 2026-05-30T13:01Z

## P0 (Critical) 🚨
- **safe_outputs add_comment validation** (#35351): **PARTIAL FIX** — PR #35901 merged 2026-05-30T12:15Z (cross-repo safe-output handlers). Verify if core `item_number` validation issue is also resolved before clearing P0.
- **Copilot CLI engine systemic failure** (#35388): All workflows using `copilot` engine fail on "Execute GitHub Copilot CLI" step. Ongoing since May 28.

## P1 (High) 🚨
- **Smoke tests systemic**: smoke-trigger, smoke-water, smoke-multi-caller all 100% failing. Issues: #35829, #35832, #35856, #35864, #35866, and more. Worsening — 6+ issues open.
- **CGO CI** (2026-05-30): Unit tests + custom linters failing (run 26675666376) — build broken.
...
- **Agentic Commands regression**: 80%→25% success, CJS correlation suspected.
- **failure-reporters duplication**: 20 issues/day, 60% duplicate rate — dedup gate unimplemented (4th escalation — needs owner NOW).
...
- safe_outputs systemic: tracked in #35351 (partial fix PR #35901)
...
- Agentic Commands regression: tracked by Workflow Health Manager
