...
const { SAFE_OUTPUT_E009 } = require("./error_codes.cjs");
    ...
    try {
      ...
      if (hasCopilot) {
        try {
          ...
          if (!pullRequestId) {
            throw new Error(`${SAFE_OUTPUT_E009}: Could not resolve pull request node ID for ${repoParts.owner}/${repoParts.repo}#${prNumber}`);
          }
