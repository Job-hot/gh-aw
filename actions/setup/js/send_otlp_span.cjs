...
async function sendJobConclusionSpan(spanName, options = {}) {
  ...
  if (jobName === "conclusion") {
    core.info(`[otlp] conclusion: built span payload with ${attributes.length} attributes`);
    core.info(`[otlp] conclusion: span name = "${spanName}"`);
    core.info(`[otlp] conclusion: trace ID = ${traceId}`);
    core.info(`[otlp] conclusion: span ID = ${conclusionSpanId}`);
    if (parentSpanId) {
      core.info(`[otlp] conclusion: parent span ID = ${parentSpanId}`);
    }
    core.info(`[otlp] conclusion: status = ${statusCode}${statusMessage ? ` (${statusMessage})` : ""}`);
