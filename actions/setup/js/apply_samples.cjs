...
const { SAFE_OUTPUT_E010, SAFE_OUTPUT_E011 } = require("./error_codes.cjs");
...
function loadSamples() {
  ...
  try {
    ...
    throw new Error(`${SAFE_OUTPUT_E010}: apply_samples: failed to parse GH_AW_SAMPLES as JSON: ${getErrorMessage(err)}`);
  }
  ...
  if (!Array.isArray(parsed)) {
    throw new Error(`${SAFE_OUTPUT_E010}: apply_samples: GH_AW_SAMPLES must be a JSON array`);
  }
  for (const [i, entry] of parsed.entries()) {
    if (!entry || typeof entry !== "object" || typeof entry.tool !== "string") {
      throw new Error(`${SAFE_OUTPUT_E010}: apply_samples: entry ${i} is missing a string "tool" field`);
    }
    if (!entry.arguments || typeof entry.arguments !== "object") {
      throw new Error(`${SAFE_OUTPUT_E010}: apply_samples: entry ${i} (tool=${entry.tool}) is missing an "arguments" object`);
    }
  }
  return parsed;
}
...
function runGit(args, cwd) {
  ...
  if (result.status !== 0) {
    throw new Error(`${SAFE_OUTPUT_E011}: git ${args.join(" ")} failed (exit ${result.status}): ${result.stderr || result.stdout}`);
  }
...
async function sendJsonRpc(child, stdin, request, responseIterator) {
  ...
  while (true) {
    ...
    if (done) {
      throw new Error(`${SAFE_OUTPUT_E011}: apply_samples: MCP server closed stdout before responding to request id=${request.id}`);
    }
    ...
    try {
      ...
      throw new Error(`${SAFE_OUTPUT_E011}: apply_samples: failed to parse MCP JSON-RPC response for request id=${request.id}: ${getErrorMessage(err)} (line: ${line})`);
    }
  }
}
...
function resolveMcpServerPath() {
  ...
  throw new Error(`${SAFE_OUTPUT_E011}: apply_samples: could not locate safe_outputs_mcp_server.cjs. Looked in: ${candidates.join(", ")}`);
}
...
async function main() {
  ...
  try {
    ...
    if (initRsp.error) {
      throw new Error(`${SAFE_OUTPUT_E011}: MCP initialize failed: ${JSON.stringify(initRsp.error)}`);
    }
  ...
  if (failures.length > 0) {
    throw new Error(`${SAFE_OUTPUT_E011}: apply_samples: ${failures.length} sample(s) failed:\n  - ${failures.join("\n  - ")}`);
  }
...
if (require.main === module) {
  main().catch(err => {
    ...
    const messageText = err && typeof err.message === "string" ? err.message : String(err);
    const hasErrorCode = /^E\d{3}:/.test(messageText);
