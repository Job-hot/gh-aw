 ...
 *   SDK "assistant.usage"         → token-usage.jsonl entry       (model, input_tokens, output_tokens, …)
 *
 ...
 *
 * Token usage (AI credits) is written to:
 *   /tmp/gh-aw/sandbox/firewall/logs/api-proxy-logs/token-usage.jsonl
 * by handling the ephemeral "assistant.usage" SDK events.  This path is the
 * same one that parse_mcp_gateway_log.cjs reads so that AI credits appear in the
 * "Generated by" footer.  In non-SDK runs the firewall proxy writes that file
 * directly; in copilot-sdk mode it is absent, so we produce it ourselves.
 *
...
// Default path where the firewall proxy normally writes per-request token usage.
// parse_mcp_gateway_log.cjs reads this file to compute AI credits.
// In copilot-sdk mode the proxy never writes here, so we write it ourselves
// by capturing the ephemeral "assistant.usage" SDK events.
// Override via GH_AW_TOKEN_USAGE_PATH (used in tests).
const DEFAULT_TOKEN_USAGE_PATH = "/tmp/gh-aw/sandbox/firewall/logs/api-proxy-logs/token-usage.jsonl";

/**
 * Append one token-usage record to the shared token-usage.jsonl file.
 * The file is created (including parent directories) on first write.
 *
 * Field names intentionally use the snake_case convention that
 * parseTokenUsageJsonl() in parse_mcp_gateway_log.cjs expects.
 *
 * @param {{ model: string, inputTokens?: number, outputTokens?: number, cacheReadTokens?: number, cacheWriteTokens?: number, reasoningTokens?: number, duration?: number }} data
 */
function appendTokenUsageEntry(data) {
  const tokenUsagePath = process.env.GH_AW_TOKEN_USAGE_PATH || DEFAULT_TOKEN_USAGE_PATH;
  const entry = {
  ...
  try {
    ...
    fs.appendFileSync(tokenUsagePath, JSON.stringify(entry) + "\n");
  } catch {
    // Best-effort: never let token-usage write failures crash the session.
  }
}
...
async function runWithCopilotSDK({ sdkUri, prompt, logger, attempt = 0, model, connectionToken, provider, maxToolDenials, permissionConfig, coreLogger, sdkModule, sessionStateBaseDir }) {
  ...
  try {
    ...
    session.on(event => {
      // "assistant.usage" carries per-request token counts.  It is marked
      // ephemeral (not persisted server-side), but we need it to write the
      // token-usage.jsonl that parse_mcp_gateway_log.cjs reads for AI credits.
      if (event.type === "assistant.usage") {
        appendTokenUsageEntry(event.data ?? {});
        return;
      }
