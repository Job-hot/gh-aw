...
func TestStrictModeGetEngineBaseEnvVarKeys_IncludesAuthSecrets(t *testing.T) {
	...
	_, hasClientID := keys["MY_CLIENT_ID"]
	assert.True(t, hasClientID, "client ID secret should be in allowed env-var keys")
	_, hasClientSecret := keys["MY_CLIENT_SECRET"]
	assert.True(t, hasClientSecret, "client secret should be in allowed env-var keys")
}
