...
func generateSafeOutputsSetup(c *Compiler, yaml *strings.Builder, safeOutputConfig string, workflowData *WorkflowData) {
	...
	yaml.WriteString("          \n")
	yaml.WriteString("          # Create initial server log file for artifact upload\n")
	yaml.WriteString("          {\n")
	yaml.WriteString("            echo \"Safe Outputs MCP Server Log\"\n")
	yaml.WriteString("            echo \"Start time: $(date)\"\n")
	yaml.WriteString("            echo \"===========================================\" \n")
	yaml.WriteString("            echo \"\"\n")
	yaml.WriteString("          } > /tmp/gh-aw/mcp-logs/safeoutputs/server.log\n")
	yaml.WriteString("          \n")
	yaml.WriteString("          # Start safe-outputs MCP server in node:lts-bookworm Docker container.\n")
	yaml.WriteString("          # node:lts-bookworm is based on buildpack-deps:bookworm which includes git,\n")
	yaml.WriteString("          # so no runtime package installation is needed.\n")
	yaml.WriteString("          # Mounts:\n")
	yaml.WriteString("          #   safeoutputs dir  – safe-outputs scripts and config files\n")
	yaml.WriteString("          #   logs dir         – write server logs back to the host\n")
	yaml.WriteString("          #   workspace        – enable git operations on the checked-out repository\n")
	yaml.WriteString("          #   gh binary        – enable gh CLI commands inside the container\n")
	yaml.WriteString("          SAFE_OUTPUTS_CONTAINER_ID=$(docker run -d \\\n")
	yaml.WriteString("            --network host \\\n")
	yaml.WriteString("            -e DEBUG \\\n")
	yaml.WriteString("            -e GH_AW_SAFE_OUTPUTS \\\n")
	yaml.WriteString("            -e GH_AW_SAFE_OUTPUTS_PORT \\\n")
	yaml.WriteString("            -e GH_AW_SAFE_OUTPUTS_API_KEY \\\n")
	yaml.WriteString("            -e GH_AW_SAFE_OUTPUTS_TOOLS_PATH \\\n")
	yaml.WriteString("            -e GH_AW_SAFE_OUTPUTS_CONFIG_PATH \\\n")
	yaml.WriteString("            -e GH_AW_MCP_LOG_DIR \\\n")
	yaml.WriteString("            -e GITHUB_TOKEN \\\n")
	yaml.WriteString("            -e GITHUB_REPOSITORY \\\n")
	yaml.WriteString("            -e GITHUB_SERVER_URL \\\n")
	yaml.WriteString("            -e GITHUB_WORKSPACE \\\n")
	yaml.WriteString("            -e GITHUB_ACTOR \\\n")
	yaml.WriteString("            -e GH_HOST \\\n")
	for _, varName := range safeOutputsConfigEnvKeys {
		yaml.WriteString("            -e " + varName + " \\\n")
	}
	yaml.WriteString("            -v \"${RUNNER_TEMP}/gh-aw/safeoutputs:${RUNNER_TEMP}/gh-aw/safeoutputs:rw\" \\\n")
	yaml.WriteString("            -v \"/tmp/gh-aw/mcp-logs/safeoutputs:/tmp/gh-aw/mcp-logs/safeoutputs:rw\" \\\n")
	yaml.WriteString("            -v \"${GITHUB_WORKSPACE}:${GITHUB_WORKSPACE}:rw\" \\\n")
	yaml.WriteString("            -v \"/usr/bin/gh:/usr/bin/gh:ro\" \\\n")
	yaml.WriteString("            --workdir \"${RUNNER_TEMP}/gh-aw/safeoutputs\" \\\n")
	fmt.Fprintf(yaml, "            %s \\\n", constants.DefaultSafeOutputsNodeImage)
	yaml.WriteString("            node mcp-server.cjs >> /tmp/gh-aw/mcp-logs/safeoutputs/server.log 2>&1)\n")
	yaml.WriteString("          echo \"Started safe-outputs MCP server container: ${SAFE_OUTPUTS_CONTAINER_ID}\"\n")
	yaml.WriteString("          \n")
	yaml.WriteString("          # Wait for server to be ready (max 60 seconds)\n")
	yaml.WriteString("          echo \"Waiting for safe-outputs MCP server to become ready...\"\n")
	yaml.WriteString("          for i in $(seq 1 60); do\n")
	yaml.WriteString("            if ! docker inspect -f '{{.State.Running}}' \"${SAFE_OUTPUTS_CONTAINER_ID}\" 2>/dev/null | grep -q true; then\n")
	yaml.WriteString("              echo \"ERROR: Safe-outputs container ${SAFE_OUTPUTS_CONTAINER_ID} has stopped\"\n")
	yaml.WriteString("              echo \"Container logs:\"\n")
	yaml.WriteString("              docker logs \"${SAFE_OUTPUTS_CONTAINER_ID}\" 2>&1 || true\n")
	yaml.WriteString("              echo \"Server log contents:\"\n")
	yaml.WriteString("              cat /tmp/gh-aw/mcp-logs/safeoutputs/server.log || true\n")
	yaml.WriteString("              exit 1\n")
	yaml.WriteString("            fi\n")
	yaml.WriteString("            if curl -s -f \"http://localhost:${GH_AW_SAFE_OUTPUTS_PORT}/health\" > /dev/null 2>&1; then\n")
	yaml.WriteString("              echo \"Safe Outputs MCP server is ready (attempt ${i}/60)\"\n")
	yaml.WriteString("              echo \"::group::Server Log Contents\"\n")
	yaml.WriteString("              cat /tmp/gh-aw/mcp-logs/safeoutputs/server.log || true\n")
	yaml.WriteString("              echo \"::endgroup::\"\n")
	yaml.WriteString("              break\n")
	yaml.WriteString("            fi\n")
	yaml.WriteString("            if [ \"${i}\" -eq 60 ]; then\n")
	yaml.WriteString("              echo \"ERROR: Safe Outputs MCP server failed to start after 60 seconds\"\n")
	yaml.WriteString("              echo \"Server log contents:\"\n")
	yaml.WriteString("              cat /tmp/gh-aw/mcp-logs/safeoutputs/server.log || true\n")
	yaml.WriteString("              exit 1\n")
	yaml.WriteString("            fi\n")
	yaml.WriteString("            echo \"Waiting for server... (attempt ${i}/60)\"\n")
	yaml.WriteString("            sleep 1\n")
	yaml.WriteString("          done\n")
	yaml.WriteString("          \n")
	yaml.WriteString("          # Write port and API key outputs for downstream steps\n")
	yaml.WriteString("          {\n")
	yaml.WriteString("            echo \"port=${GH_AW_SAFE_OUTPUTS_PORT}\"\n")
	yaml.WriteString("            echo \"api_key=${GH_AW_SAFE_OUTPUTS_API_KEY@Q}\"\n")
	yaml.WriteString("          } >> \"$GITHUB_OUTPUT\"\n")
	yaml.WriteString("          \n")
}
