...
func generateSafeOutputsSetup(c *Compiler, yaml *strings.Builder, safeOutputConfig string, workflowData *WorkflowData) {
	...
	yaml.WriteString("          \n")
	yaml.WriteString("          # Start safe-outputs MCP server in node:lts-alpine Docker container.\n")
	...
	yaml.WriteString("            sh -c 'apk add --no-cache git >/dev/null 2>&1 && node mcp-server.cjs >> /tmp/gh-aw/mcp-logs/safeoutputs/server.log 2>&1')\n")
