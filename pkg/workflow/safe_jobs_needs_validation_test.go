...
func TestComputeValidSafeJobNeeds(t *testing.T) {
	t.Run("base – no safe-outputs", func(t *testing.T) {
		...
		assert.Contains(t, valid, "agent", "agent should always be valid")
		assert.NotContains(t, valid, "detection", "detection not valid without safe-outputs")
		assert.NotContains(t, valid, "safe_outputs", "safe_outputs not valid without safe-outputs")
	})
	...
	t.Run("only custom jobs configured – safe_outputs absent", func(t *testing.T) {
		...
		valid := computeValidSafeJobNeeds(data)
		assert.NotContains(t, valid, "safe_outputs", "safe_outputs should not be valid when only custom jobs present")
	})
	...
	t.Run("builtin type configured – safe_outputs present", func(t *testing.T) {
		...
		valid := computeValidSafeJobNeeds(data)
		assert.Contains(t, valid, "agent")
		assert.Contains(t, valid, "safe_outputs")
		assert.Contains(t, valid, "detection", "detection enabled when ThreatDetection is non-nil")
		assert.NotContains(t, valid, "upload_assets")
		assert.NotContains(t, valid, "unlock")
	})
	...
	t.Run("with upload-asset configured", func(t *testing.T) {
		...
		valid := computeValidSafeJobNeeds(data)
		assert.Contains(t, valid, "upload_assets")
	})
	...
	t.Run("with lock-for-agent enabled", func(t *testing.T) {
		...
		valid := computeValidSafeJobNeeds(data)
		assert.Contains(t, valid, "unlock")
	})
	...
	t.Run("custom safe-job names are included", func(t *testing.T) {
		...
		valid := computeValidSafeJobNeeds(data)
		assert.Contains(t, valid, "my_packager", "dash-to-underscore normalized name should be valid")
		assert.Contains(t, valid, "notify_team")
	})
}
