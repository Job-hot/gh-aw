...
func (c *Compiler) generateAgentRunSteps(yaml *strings.Builder, data *WorkflowData, engine CodingAgentEngine, needsGitConfig bool) ([]string, string, error) {
	...
	artifactPaths = append(artifactPaths, constants.AgentPromptFilePath)
...
func (c *Compiler) collectArtifactPaths(data *WorkflowData, engine CodingAgentEngine, logFileFull string, initialPaths []string) []string {
	...
	// Collect MCP logs.
	paths = append(paths, constants.MCPLogsDir+"/")
	...
	if IsMCPScriptsEnabled(data.MCPScripts) {
		paths = append(paths, constants.MCPScriptsLogsDir+"/")
	}
	...
	if isFirewallEnabled(data) {
		paths = append(paths, constants.TmpGhAwDir+"/"+constants.TokenUsageFilename)
	}
	...
	if isOTLPEnabled(data) {
		paths = append(paths, constants.TmpGhAwDir+"/"+constants.OtelJsonlFilename)
		paths = append(paths, constants.TmpGhAwDir+"/"+constants.OtlpExportErrorsFilename)
	}
	...
	if data.SafeOutputs != nil {
		...
		paths = append(paths, constants.TmpGhAwDir+"/"+constants.SafeOutputsFilename)
		// Processed agent output JSON produced by collect_ndjson_output.cjs
		paths = append(paths, constants.TmpGhAwDir+"/"+constants.AgentOutputFilename)
		if data.SafeOutputs.CommentMemory != nil {
			paths = append(paths, constants.CommentMemoryDir+"/")
		}
	}
	...
	if usesPatchesAndCheckouts(data.SafeOutputs) || threatDetectionNeedsPatches {
		paths = append(paths, constants.AwPatchGlob)
		...
		paths = append(paths, constants.AwBundleGlob)
	}
