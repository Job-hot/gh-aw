...
func ValidateDoc(doc SSLDoc) []string {
	...
	sceneIDs := make(map[string]struct{}, len(doc.Scenes))
	for _, s := range doc.Scenes {
		sceneIDs[s.ID] = struct{}{}
	}
	stepIDs := make(map[string]struct{}, len(doc.LogicSteps))
	for _, ls := range doc.LogicSteps {
		stepIDs[ls.ID] = struct{}{}
	}
	// Rule 1: entry_scene must reference an existing scene.
	if doc.Scheduling.EntryScene != "" {
		if _, ok := sceneIDs[doc.Scheduling.EntryScene]; !ok {
	...
	for _, scene := range doc.Scenes {
		...
		if scene.EntryLogicStep != "" {
			if _, ok := stepIDs[scene.EntryLogicStep]; !ok {
		...
		for _, rule := range scene.NextSceneRules {
			if _, ok := sceneIDs[rule.Target]; !ok {
				if _, termOk := sceneTerminals[rule.Target]; !termOk {
	...
	for _, step := range doc.LogicSteps {
		...
		if _, ok := stepIDs[step.Next]; !ok {
			if _, termOk := stepTerminals[step.Next]; !termOk {
				msgs = append(msgs, fmt.Sprintf(
