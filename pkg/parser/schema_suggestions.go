...
func addObjectExamples(result map[string]any, properties map[string]any, requiredFields map[string]struct{}, includeRequired bool, count int) int {
	for propName, propSchema := range properties {
		_, isRequired := requiredFields[propName]
		if isRequired != includeRequired || count >= maxExampleFields {
			continue
		}
