...
process.stderr.write("[custom-copilot-driver] custom sample driver active\n");
...
/**
 * Log a message prefixed with [custom-copilot-driver] to stderr.
 * @param {string} msg
 */
function log(msg) {
  process.stderr.write(`[custom-copilot-driver] ${msg}\n`);
}
...
async function main() {
  ...
  if (!promptFile) {
    process.stderr.write("[custom-copilot-driver] error: GH_AW_PROMPT is not set\n");
    process.exit(1);
  }

  const sdkUri = process.env.COPILOT_SDK_URI;
  if (!sdkUri) {
    process.stderr.write("[custom-copilot-driver] error: COPILOT_SDK_URI is not set\n");
    process.exit(1);
  }
  ...
  if (!connectionToken) {
    process.stderr.write(
      "[custom-copilot-driver] error: COPILOT_CONNECTION_TOKEN is required. This token is generated by copilot_harness.cjs and must be passed to the driver environment\n"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write(`[custom-copilot-driver] error: failed to read prompt file ${promptFile}: ${err}\n`);
    process.exit(1);
  }
...
if (require.main === module) {
  main().catch(err => {
    process.stderr.write(`[custom-copilot-driver] unhandled error: ${err instanceof Error ? err.stack : String(err)}\n`);
    process.exit(1);
  });
}
