...
| Tool | Purpose | Key Parameters |
|------|---------|----------------|
| `check_dependency_vulnerabilities` | Check a list of dependencies against the GitHub Advisory Database for known vulnerabilities | `owner`, `repo`, `dependencies` (with `name`, `version`, `ecosystem`) |
