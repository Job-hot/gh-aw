...
- **`max-effective-tokens:`** — Per-run ET budget enforced by the AWF firewall/API proxy (default `10000000`). The agent is steered to stay within budget; set a negative value to disable enforcement and steering.
- **`max-daily-effective-tokens:`** — Per-user 24-hour guardrail. At activation, gh-aw sums the triggering user's ET across their runs of this workflow over the last 24 hours and blocks execution once the total exceeds the threshold. Defaults to `50000000`; set `-1` to disable, or an explicit value to override the default.
