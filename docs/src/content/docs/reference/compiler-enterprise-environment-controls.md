...
```bash
gh variable set GH_AW_DEFAULT_MAX_EFFECTIVE_TOKENS --org my-org --body "10M"
```
...
```bash
gh variable set GH_AW_DEFAULT_MAX_DAILY_EFFECTIVE_TOKENS --org my-org --body "50M"
```

Set compiler process defaults for timeout and max-turns:

```bash
export GH_AW_DEFAULT_MAX_DAILY_EFFECTIVE_TOKENS=50M
