...
Compiler-generated concurrency groups (agent and conclusion jobs) emit `queue: max` by default so back-to-back triggers run sequentially rather than being dropped. Set `features.group-concurrency-queue: false` to omit `queue` from generated groups and revert to the Actions default:
...
GitHub Enterprise Server (GHES) does not support the `queue` field in concurrency blocks. If you're deploying to GHES, set `features.group-concurrency-queue: false` in your workflow frontmatter to disable `queue: max` generation.
:::
