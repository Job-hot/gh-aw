...
|---|---|---|
| `Experiment` | `name:string`, `variants:string[]`, `metric:string?`, `min_samples:int?`, `weight:int[]?` | Declarative experiment definition from frontmatter. |
| `Variant` | `name:string`, `index:int`, `weight:int?`, `is_control:bool` | One treatment arm in an experiment definition. |
| `Assignment` | `run_id:string`, `experiment:string`, `variant:string`, `timestamp:string` | Per-run selection record used by activation and reporting flows. |
| `ExperimentState` | `experiments:object`, `runs:array`, `updated_at:string` | Persisted state snapshot stored in `state.json`. |
| `ActivationJob` | `job_id:string`, `steps:array`, `outputs:object`, `permissions:object` | Generated workflow job that selects variants and publishes assignment outputs/artifacts. |
...
### 14.3 MUST Compliance Matrix

| Requirement | Test ID | Level | Status |
|---|---|---|---|
| R-SCHEMA-001 | T-SCHEMA-006 | 1 | Required |
| R-SCHEMA-002 | T-SCHEMA-002 | 1 | Required |
| R-SCHEMA-005 | T-SCHEMA-001 | 1 | Required |
| R-SCHEMA-007 | T-SCHEMA-003 | 1 | Required |
| R-SELECT-001 | T-SELECT-001 | 1 | Required |
| R-SELECT-003 | T-SELECT-003 | 1 | Required |
| R-SELECT-006 | T-SELECT-005 | 1 | Required |
| R-EXPR-001 | T-EXPR-001 | 1 | Required |
...
This section maps key normative sections to the implementation files used to keep this specification synchronized with runtime behavior.

| Spec section | Scope | Primary implementation file(s) |
|---|---|---|
| §4 Frontmatter Schema | `experiments:` frontmatter parsing and type model | `pkg/workflow/frontmatter_types.go`, `pkg/workflow/compiler_experiments.go`, `pkg/workflow/compiler_validators.go` |
| §5 Variant Selection Algorithms | Balanced/weighted variant assignment behavior | `actions/setup/js/pick_experiment.cjs`, `actions/setup/js/experiment_helpers.cjs`, `pkg/workflow/compiler_activation_job.go` |
| §7 State Persistence | `state.json` lifecycle and repo/cache persistence flow | `actions/setup/js/load_experiment_state_from_repo.cjs`, `actions/setup/js/push_experiment_state.cjs`, `pkg/workflow/compiler_activation_job.go` |
| §10 Audit CLI Integration | Experiment/variant filtering and assignment lookup in audit output | `pkg/cli/audit_report_experiments.go`, `pkg/cli/experiments_command.go`, `pkg/cli/audit_report.go` |
