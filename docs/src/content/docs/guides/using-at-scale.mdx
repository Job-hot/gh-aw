...
[Reusing Workflows](/gh-aw/guides/packaging-imports/) covers the developer-facing mechanics: use `gh aw add` or `gh aw add-wizard` to install a workflow from another repository, and `gh aw update` to pull in upstream changes while preserving local edits. Use `imports:` in workflow frontmatter to pull in shared components (tool configs, MCP server definitions, prompt snippets) — see [Imports](/gh-aw/reference/imports/) for the full reference.
