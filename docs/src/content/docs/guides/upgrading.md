...
Check your version with `gh aw version` and compare against the [latest release](https://github.com/github/gh-aw/releases). By default, `gh aw upgrade` follows stable releases; use `--pre-release` to opt into pre-release builds. For example, `gh aw upgrade --pre-release` installs the latest pre-release build. If you encounter issues, try a clean reinstall with `gh extension remove gh-aw` followed by `gh extension install github/gh-aw`.
